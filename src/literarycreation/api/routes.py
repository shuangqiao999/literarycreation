...
async def start_optimize(session_id: str, req: OptimizeRequest, request: Request):
    engine = _get_engine(request)
    session = engine.get_session(session_id)
    if session is None:
        raise HTTPException(404, "Session not found")
    if not req.scenarios:
        raise HTTPException(400, "请提供至少一个对比方案")
    cancels = _ded_cancel_state(request.app)
    if session_id in cancels and not cancels[session_id].is_set():
        raise HTTPException(409, "该会话已有任务运行中")
    cancel_event = asyncio.Event()
    cancels[session_id] = cancel_event
    try:
        asyncio.create_task(_run_optimize(engine, session_id, req.scenarios, cancel_event, cancels))
        return {"session_id": session_id, "status": "optimizing"}
    except Exception as e:
        cancels.pop(session_id, None)
        raise HTTPException(500, str(e))
...
async def cancel_optimize(session_id: str, request: Request):
    cancels = _ded_cancel_state(request.app)
    ev = cancels.get(session_id)
    if ev:
        ev.set()
...
async def get_optimize_result(session_id: str, request: Request):
    engine = _get_engine(request)
    session = engine.get_session(session_id)
    if session is None:
        raise HTTPException(404, "Session not found")
...
async def _run_optimize(engine, session_id: str, scenarios: list, cancel_event, cancels: dict):
    try:
        from literarycreation.engine.optimizer import StrategyOptimizer
        from literarycreation.engine.models import SessionStatus
        import json
        ...
        orchestrator_data = engine.session_store.get(session_id)
        agent_data = []
        ...
        if not agents:
            engine.log(session_id, "optimizer", "无法重建角色列表，请先完成推演后再运行优化")
            engine.session_store.update(session_id, status=SessionStatus.COMPLETE.value)
            return
        ...
        if orchestrator_data:
            cfg_full = orchestrator_data.get("config_json") or {}
            if isinstance(cfg_full, str):
                cfg_full = json.loads(cfg_full)
            snapshot = cfg_full.get("state_snapshot") or {}

        if snapshot and snapshot.get("states"):
            from literarycreation.engine.models import EntityState
            for eid, raw in snapshot["states"].items():
                st = EntityState(
                    id=raw.get("id", eid),
        ...
        if not states:
            engine.log(session_id, "optimizer", "无法重建状态快照，请先完成推演")
            engine.session_store.update(session_id, status=SessionStatus.COMPLETE.value)
            return
        ...
        try:
            ...
            domain = str(cfg.get("domain", "literary_realism") or "literary_realism")
            rule_engine = RuleEngine.from_domain(domain)
        except Exception:
            pass

        engine.session_store.update(session_id, status=SessionStatus.OPTIMIZING.value)

        def progress_fn(idx, total, detail):
            engine.signal_optimize_progress(session_id, idx, total, detail)

        results, recommendation = await opt.run(
            ...
            log_fn=lambda p, m: engine.log(session_id, p, m),
        ...
        engine.log(session_id, "optimizer", f"优化完成: {len(results)} 方案已对比")

    except Exception:
        import logging
        logging.getLogger(__name__).exception("[LiteraryCreation] optimize failed")
        try:
            engine.session_store.update(session_id, status=SessionStatus.FAILED.value)
        except Exception:
            pass
    finally:
        cancels.pop(session_id, None)
...
async def update_settings(session_id: str, req: SettingsRequest, request: Request):
    ...
    engine.session_store.update(session_id, config_json=json.dumps(config, ensure_ascii=False))
    return {"session_id": session_id, "weather": config["weather"], "terrain": config["terrain"]}
...
async def stream_deduction(session_id: str, request: Request):
    ...
    async def event_generator():
        ...
        while True:
            ...
            # ── push optimize progress event ──
            od = engine.get_optimize_data(session_id)
            oidx = od.get("idx", 0)
            if oidx > getattr(engine, '_last_optimize_idx', {}).get(session_id, 0):
                if not hasattr(engine, '_last_optimize_idx'):
                    engine._last_optimize_idx = {}
                engine._last_optimize_idx[session_id] = oidx
                opt_payload: dict[str, Any] = {
                    "type": "optimize_progress",
                    "idx": oidx,
                    "total": od.get("total", 0),
                }
                detail = od.get("detail") or {}
                if detail:
                    opt_payload["detail"] = detail
                yield f"data: {json.dumps(opt_payload, ensure_ascii=False)}\n\n"
