...
def _strip_meta_lines(text: str) -> str:
    """剔除模型输出中的元信息行：章节标题、markdown 标题、分隔线、[内心]/[场景]标记。"""
    import re
    global _HEADING_RE
    if _HEADING_RE is None:
        _HEADING_RE = re.compile(
            r"^(?:#{1,6}\s.*|(?:#{1,6}\s*)?第\s*[0-9一二三四五六七八九十百零两]+\s*章(?:[:：\s].*)?)$")
    out: list[str] = []
    for line in text.split("\n"):
        s = line.strip()
        if s in {"***", "---", "* * *", "＊＊＊"} or _HEADING_RE.match(s):
            continue
        out.append(line)
    cleaned = "\n".join(out)
    cleaned = cleaned.replace("[内心]", "").replace("[场景]", "")
...
- {length_req}
- 【情节优先】以推进剧情为第一目标。每个场景只写一次，写完即向前推进；严禁回放、复述或用不同措辞重写本章或前文已出现的场景与独白。
- 【语言凝练】删除冗余修饰与同义反复；同一比喻、同一生理反应描写（如"瞳孔收缩""屏住呼吸"）全章至多出现一次；连续的环境渲染不超过两段。
...
- 【避免口头禅】角色的座右铭或信仰（如'活着的人不走死人的路'）只能在本章出现1次。如果前几章已多次出现，本章请用行动而非重复同一句话来表达角色的信念。
...
async def _retry_prose(client, prompt, story_context, target_words, chapter_idx):
    ...
    # 字数为软目标：上限给足冗余，避免截断情节需要的长章
    max_tok_full = max(8192, int(target_words * 2.4)) if target_words and target_words > 0 else 0
    max_tok_half = max(4096, int(target_words * 1.2)) if target_words and target_words > 0 else 0

    strategies = [
        (prompt, "你是文学作家，逐章创作小说/剧本正文，文笔凝练、情节紧凑、承接自然。",
         0.85, max_tok_full),
        (prompt, "你是文学作家，输出小说正文。情节紧凑，不重复任何已写场景。", 0.5, max_tok_half),
        (f"【当前剧情进度】\n{story_context[:800]}\n\n请用300字续写，推进主线剧情：",
...
def build_story_context(story_state: dict, chapter_idx: int) -> str:
    ...
    if recent:
        parts.append("\n## 最近章节进展\n" + "\n".join(recent))
        # 反场景重复：检测连续两章有无相似的场景模式，如有则警告
        if len(recent) >= 2:
            recent_two = " ".join(recent[-2:])
            repetitive_patterns = [
                ("暗格", "请勿再让角色在本章发现暗格或翻动砖块——这种场景模式"
                          "已在前两章连续出现，读者会产生厌倦。本章优先让角色与人互动、"
                          "做决定或面对后果"),
                ("脚印", "脚印/足迹已是前两章的核心推动元素。本章请用对话、揭示或"
                          "实际行动推动剧情，不再依赖雪地上的足迹"),
                ("密信", "前两章均出现了密信/油布包裹。本章请用已获得的线索做决策，"
                          "不再增加新的秘密文件"),
                ("墙|窗", "前两章角色都在观察/贴墙/看窗外。本章请让角色做出一个"
                          "不可逆的动作——走进房间、面对敌人、打破沉默"),
            ]
            for keyword, advice in repetitive_patterns:
                words_found = sum(1 for w in keyword.split("|") if w in recent_two)
                if words_found >= 2:
                    parts.append(f"\n【情节推进 — 请注意避免场景重复】\n{advice}")
                    break
        ...
        *,
        ...
        for e in round_events:
            if "[场景]" in e:
                ...
                raw = e.split("[内心]", 1)[-1].strip()[:80]
                frag = f"（心理素材）{raw}" if raw else ""
            else:
                continue
            key = frag[:30]
            if frag and key not in _seen_frag:
                _seen_frag.add(key)
                scene_fragments.append(frag)
        if len(scene_fragments) >= 2:
            scene_block = (
                "【场景素材（仅供参考，不得照抄）——将其融合改写为一段连续推进的叙事，"
                "每条素材至多化用一次，严禁逐字照抄、严禁同一素材反复出现；"
                "严禁在正文中保留[内心]、[场景]等任何标记；"
                "心理素材须转化为视角人物的言行或叙述，非视角角色的内心不得直接呈现】\n"
        ...
        length_req = (
            f"篇幅由情节需要决定（参考量级约 {target_words} 字，不作硬性要求）——"
            "情节推进完毕即收束本章，宁短勿长，严禁为凑篇幅注水、复述或改写已写过的场景。"
            if target_words and target_words > 0 else
            "篇幅由情节需要决定——情节推进完毕即收束本章，以情节紧凑、语言凝练为先，严禁注水。")
